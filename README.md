...
- Detecta columnas de identificación, nombres y calificación a partir de sus encabezados.
- Busca coincidencias por RUT/ID y por nombres, normalizando mayúsculas, tildes, separadores y el orden de las palabras.
- Conserva las primeras tres columnas y el orden de las filas de MiPortal; agrega una columna `nota`.
...
Usa el listado del curso con al menos tres columnas. La disposición esperada es **N, Rut, Nombre**. La aplicación conserva únicamente las primeras tres columnas, incluidos sus encabezados, y agrega la nota al final.
...
La columna de nota se selecciona automáticamente: se priorizan encabezados como `nota`, `calificación`, `grade`, `score`, `puntaje`, `resultado` o `promedio`, luego la cantidad de valores numéricos y, en caso de empate, la columna situada más a la derecha. **Si hay varias evaluaciones, prepara el archivo con una sola columna de calificación y revisa el resultado.**
...
3. Pulsa **Procesar y generar archivo**.
4. Revisa la tabla, el número de coincidencias y el detalle de estudiantes sin coincidencia.
5. Usa **Descargar resultado** para obtener el `.xlsx`, o **Copiar notas** para copiar una nota por línea en el orden del listado. Las celdas de nota vacías se omiten al copiar.
6. Traspasa las notas a MiPortal después de verificar los resultados.

Puedes reemplazar uno de los archivos y volver a procesar con el otro ya cargado. **Borrar archivo** elimina ese archivo y el resultado asociado; **Borrar todo** elimina ambos archivos cargados y el resultado de la sesión.
